...
__global__ void receive_packets_kernel_persistent(int rxqn, uintptr_t* eth_rxq_gpu,
  ...
  do {
    ...
    while (buf_idx < rx_pkt_num) {
      ret = DOCA_ERROR_UNKNOWN;doca_gpu_dev_eth_rxq_get_buf(rxq, rx_buf_idx + buf_idx, &buf_ptr);
      if (ret != DOCA_SUCCESS) {
      ...
      ret = DOCA_ERROR_UNKNOWN;doca_gpu_dev_buf_get_addr(buf_ptr, &buf_addr);
      if (ret != DOCA_SUCCESS) {
        printf("UDP Error %d doca_gpu_dev_eth_rxq_get_buf thread %d\n", ret, threadIdx.x);
        DOCA_GPUNETIO_VOLATILE(*exit_cond) = 1;
        break;
...
__global__ void send_packets_kernel(struct doca_gpu_eth_txq* txq, struct doca_gpu_buf_arr* buf_arr,
  ...
  while (pkt_idx < num_pkts) {
    ...
    ret = DOCA_ERROR_UNKNOWN;doca_gpu_dev_buf_get_buf(buf_arr, ((pkt_idx + gpu_pkt0_idx) % max_pkts), &buf);
    if (ret != DOCA_SUCCESS) {
